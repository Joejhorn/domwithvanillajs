"use strict";
{
  ...
  const init = () => {
    ...
    document.querySelector('#firstArticle').addEventListener('click', showFirstMessage);
    document.querySelector('#lastArticle').addEventListener('click', lastMessage);
    document.body.addEventListener('keydown', checkKey);
  };
  const checkKey = (e) => {
    if (e.altKey) {
      if (e.key === "ArrowRight") lastMessage();
      if (e.key === "ArrowLeft") showFirstMessage();
    } else {
      if (e.keyCode == '37') {
        prevMessage();
  ...
  const showNumberOfMessages = () => {
    //with qs instead of getElementsByClassName
    // document.querySelector('.message_number').innerText = numberOfMessages();
    // var span = document.getElementsByClassName("galleria-current")[0]
    document.getElementsByClassName('message_number')[0].innerHTML = numberOfMessages();
  }
  ...
  const lastMessage = () => {
    currentMessageNumber = numberOfMessages();
    showMessageByNumber(currentMessageNumber);
